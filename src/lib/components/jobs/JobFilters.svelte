...
<div class="flex gap-2 overflow-x-auto pb-0.5 scrollbar-none">
  {#each statusOptions as opt (opt.label)}
    <button
      class="shrink-0 cursor-pointer rounded-full px-3 py-1 text-sm transition-colors {chipClass(
        filters.status === opt.value,
      )}"
      onclick={() => onChange({ ...filters, status: opt.value, cursor: null })}
    >
      {opt.label}
    </button>
  {/each}
  ...
  {#each typeOptions as opt (opt.label)}
    <button
      ...
      onclick={() => onChange({ ...filters, generation_type: opt.value, cursor: null })}
    >
      {opt.label}
    </button>
  {/each}
