<script lang="ts">
  ...
  const supportsTier = $derived(isGenerationParameterSupported(modelInfo, 'image_resolution'));
  const supportsCustomSize = $derived(
    isGenerationParameterSupported(modelInfo, 'width') &&
      isGenerationParameterSupported(modelInfo, 'height'),
  );
  const sizingModes = $derived(supportedSizingModes(modelInfo));
  const supportsSeed = $derived(isGenerationParameterSupported(modelInfo, 'seed'));
  const supportsSteps = $derived(isGenerationParameterSupported(modelInfo, 'steps'));
  const supportsCfg = $derived(isGenerationParameterSupported(modelInfo, 'cfg'));
  const supportsSampler = $derived(isGenerationParameterSupported(modelInfo, 'sampler'));
  const supportsScheduler = $derived(isGenerationParameterSupported(modelInfo, 'scheduler'));
  const supportsDenoise = $derived(isGenerationParameterSupported(modelInfo, 'denoise'));
  ...
  // Discovery can change beneath an open Create draft. Keep the selected
  // mechanism valid so the only visible control always serializes its value.
  $effect(() => {
    const modes = sizingModes;
    if (!modes.includes($generationStore.sizingMode) && modes.length > 0) {
      generationStore.setSizingMode(modes[0]);
    }
  });
</script>
...
<div class="flex flex-col gap-3">
  <!-- Sizing mode toggle -->
  {#if supportsTier || supportsCustomSize}
    <div class="flex flex-col gap-2">
      ...
      <div class="flex gap-1.5">
        {#each ['tier', 'custom'] as const as mode (mode)}
          ...
          {#if (mode === 'tier' && supportsTier) || (mode === 'custom' && supportsCustomSize)}
            <button
    ...
    <!-- Tier mode -->
    {#if sizingMode === 'tier' && supportsTier}
